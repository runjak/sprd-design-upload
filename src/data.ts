...
import {
  Assortment, CurrenciesData,
  Idea,
  Ideas, IdeaTranslation, PointOfSale, PointOfSaleType, PointsOfSale
} from "./types";

export function findIdForIsoCodeInCurrenciesData(
  currenciesData: CurrenciesData,
  wantedIsoCode: string = "EUR"
): string | null {
  const { currencies } = currenciesData;

  const [currency] = currencies.filter(
    ({ isoCode }) => isoCode === wantedIsoCode
  );
...
export function newestIdea(ideas: Ideas): Idea | null {
  return ideas.list.reduce((last: Idea | null, next: Idea) => {
    if (last === null) {
      return next;
    }
...
export function setPublishingDetails(
  idea: Idea,
...
export function filterPointsOfSaleByType(
  pointsOfSale: PointsOfSale,
  filterType: PointOfSaleType
): Array<PointOfSale> {
...
export function designUrlForIdea(
  idea: Idea,
  productTypeId: string = "812"
): string {
  const pointsOfSale = (idea.publishingDetails || []).map((p) => p.pointOfSale);
  const [shopName = ""] = pointsOfSale
